...
# --- OPD: admission pacer (cohort de-synchronisation) ------------------------------------------
# Chunk streaming re-submits every row as a NEW engine request at each chunk boundary
# (single_turn_agent_loop.py:263-265 -> llm_server.py:292 "use new request_id for each turn"),
# turning vLLM's FCFS run-to-completion into fair-share round-robin: all in-flight rows then
# advance at the same token rate, so parents admitted together also FINISH together, and the
# completion-clocked admission loop re-injects that cohort indefinitely. Upstream veRL issues ONE
# request per row, so FCFS queueing destroys the cohort within a generation; the streaming path has
# no such decoherence mechanism, which is why the trainer starves periodically (~1 stall per
# max_concurrent_samples*rollout.n/target_rows updates) while the TEACHER IS IDLE.
# OPD_ADMIT_PACE_S > 0 spaces successive parent dispatches by at least that many seconds, giving
# the in-flight population permanent phase dispersion. The delay is BYPASSED whenever fewer than
# OPD_ADMIT_PACE_MIN_INFLIGHT parents are in flight, so it cannot idle the rollout engine.
# Unset / 0 => disabled: the admission path is then identical to the published runs.
_OPD_ADMIT_PACE_S = float(os.environ.get("OPD_ADMIT_PACE_S", "0") or 0)
_OPD_ADMIT_PACE_JITTER = float(os.environ.get("OPD_ADMIT_PACE_JITTER", "0.25") or 0)
...
class FullyAsyncRollouter(SeparateRayPPOTrainer):
    ...
    def __init__(
        ...
        # Admission pacer state (see OPD_ADMIT_PACE_S). Inert unless the env var is set.
        self._admit_last_ts = 0.0
        self._admit_pace_min_inflight = 0
        self._admit_pace_total_delay_s = 0.0
        self._admit_pace_n_delayed = 0
    ...
    async def set_max_required_samples(self):
        async with self.lock:
            ...
            # running slot = ceil(replicas * max_num_seqs / rollout.n).
            if _OPD_ADMIT_PACE_S > 0:
                _rc = self.config.actor_rollout_ref.rollout
                try:
                    _mns = int(_rc.get("max_num_seqs", 16) or 16)
                except Exception:
                    _mns = 16
                _slots = len(self.llm_server_manager.get_replicas()) * _mns
                _n = max(1, int(_rc.n))
                _floor_env = os.environ.get("OPD_ADMIT_PACE_MIN_INFLIGHT", "").strip()
                self._admit_pace_min_inflight = (
                    int(_floor_env) if _floor_env else max(1, (_slots + _n - 1) // _n)
                )
                print(
                    f"[FullyAsyncRollouter][AdmitPacer] ENABLED min_gap={_OPD_ADMIT_PACE_S:.2f}s "
                    f"jitter=+/-{_OPD_ADMIT_PACE_JITTER:.0%} "
                    f"bypass_below_inflight={self._admit_pace_min_inflight} "
                    f"(running_slots={_slots}, rollout.n={_n})",
                    flush=True,
                )
    ...
    async def _processor_worker(self):
        ...
        while True:
            ...
            # --- OPD admission pacer (default OFF; see OPD_ADMIT_PACE_S) ---------------------
            # Space consecutive dispatches so the in-flight population does not stay phase
            # coherent. Bypassed while in-flight < floor => cannot starve the rollout engine.
            # Touches ONLY dispatch timing: no label, mask, policy_version, span, carrier, queue
            # or teacher state is read or written here.
            if _OPD_ADMIT_PACE_S > 0 and len(self.active_tasks) >= self._admit_pace_min_inflight:
                _gap = _OPD_ADMIT_PACE_S
                if _OPD_ADMIT_PACE_JITTER > 0:
                    _gap *= 1.0 + random.uniform(-_OPD_ADMIT_PACE_JITTER, _OPD_ADMIT_PACE_JITTER)
                _delay = self._admit_last_ts + _gap - time.time()
                if _delay > 0:
                    self._admit_pace_total_delay_s += _delay
                    self._admit_pace_n_delayed += 1
                    if self._admit_pace_n_delayed % 50 == 1:
                        print(
                            f"[FullyAsyncRollouter][AdmitPacer] delayed={self._admit_pace_n_delayed} "
                            f"total_delay={self._admit_pace_total_delay_s:.1f}s "
                            f"inflight={len(self.active_tasks)}",
                            flush=True,
                        )
