...
class GlobalRequestLoadBalancer:
    ...
    def __init__(self, servers: dict[str, ray.actor.ActorHandle], max_cache_size: int = DEFAULT_ROUTING_CACHE_SIZE,
        ...
        self._group_to_server: LRUCache = LRUCache(maxsize=max_cache_size)  # prompt-group -> server (prompt_affinity)
        ...
        #   prompt_affinity-> co-locate a prompt's rollout siblings on one replica so the shared PROMPT
        #                     prefix is scored once and reused across samples; different prompts spread by
        #                     least-in-flight. Assumes one prompt per rollout step (gen_batch_size=1), so
        #                     the "_r<n>"-stripped request id is the prompt-group key.
        ...
        self._policy: str = policy if policy in ("inflight", "parents", "future_debt", "prompt_affinity") else "inflight"
    ...
    def acquire_server(self, request_id: str, track_parent: bool = False) -> tuple[str, ray.actor.ActorHandle]:
        ...
        if track_parent and self._policy == "prompt_affinity":
            # Route a prompt's rollout siblings to one replica so the shared PROMPT prefix is scored
            # once and its KV reused across samples; a NEW prompt spreads by least-in-flight.
            group_key = re.sub(r"_r\d+$", "", request_id)
            gsrv = self._group_to_server.get(group_key)
            if gsrv is not None and gsrv in self._inflight_requests:
                server_id = gsrv
            else:
                server_id = min(self._inflight_requests, key=self._inflight_requests.get)
                self._group_to_server[group_key] = server_id
        elif track_parent and self._policy == "parents":
